...
fn fence_repair_mismatched_lengths() {
    ...
    let result = ascfix::modes::process_by_mode(&ascfix::cli::Mode::Safe, &input, true);
...
fn fence_repair_idempotent_mismatched() {
    ...
    // Process twice
    let first = ascfix::modes::process_by_mode(&ascfix::cli::Mode::Safe, &input, true);
    let second = ascfix::modes::process_by_mode(&ascfix::cli::Mode::Safe, &first, true);
...
fn fence_repair_with_diagram_mode() {
    ...
    let result = ascfix::modes::process_by_mode(&ascfix::cli::Mode::Diagram, input, true);
...
fn fence_repair_disabled_by_default() {
    ...
    let result = ascfix::modes::process_by_mode(&ascfix::cli::Mode::Safe, input, false);
