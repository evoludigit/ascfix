...
fn edge_case_single_character() {
    // Minimal valid input
    let input = "```\n┌┐\n└┘\n```";
    let result = ascfix::modes::process_by_mode(&ascfix::cli::Mode::Diagram, input, false);
...
fn edge_case_text_rows_preservation() {
    ...
    let result = ascfix::modes::process_by_mode(&ascfix::cli::Mode::Diagram, &golden_input, false);
